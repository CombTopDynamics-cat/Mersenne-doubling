...
void Help(char * progname) {
  fprintf (stderr, "\nUsage: %s q_ini q_end\n\nThis program works only for odd Qs larger than 4.\nThus, q_ini and q_end must be larger than 4.\n\n\n", progname);
  exit(1);
}
...
unsigned long int period (unsigned long int q) {
   ...
   unsigned long int t      = floor_log2(q),
                     a      = (1UL << t),
                     r      = a - (q-a),
                     period = t+1;

   while (r != 1) {
     if (r > r_boundary) { t = 0; a = r; while (a <= qh) { a += a; t++; } }
     else { t = floor_log2((q-1) / r); a = (r << t); }
     r = a - (q-a); period += (t+1);
   }
   return period;
...
int main (int argc, char *argv[]) {
    ...
    if (q_ini < 5 || q_end < 5) Help(argv[0]);
