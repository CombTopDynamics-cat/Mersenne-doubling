...
void Help(char * progname) {
  fprintf (stderr, "\nUsage: %s [-h] [--help] filename field_number\n", progname);
  fputs ("\nPARAMETERS:\n\
    filename     is the full path of a file containing a column of \n\
                 non-negative integers to be checked for primality.\n\
                 The filename must not begin with the character '-'.\n\
                 The column separators are strings composed of any\n\
                 number of spaces or tabs.\n\
                 The lines starting with the character '#' or without\n\
                 a field field_number or with a non-numeric field\n\
                 field_number will be considered comment lines\n\n\
    field_number is a positive integer specifying the number of the\n\
